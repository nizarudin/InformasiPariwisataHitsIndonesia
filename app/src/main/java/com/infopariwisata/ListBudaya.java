...
public class ListBudaya extends ListActivity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.list_wisata_budaya);
		
		database = new Database(this);
		database.bukaKoneksi();
		Cursor cursor = database.bacaWisataBudaya();

		startManagingCursor(cursor);
		String[] awal = new String[] { database.FIELD_JUDUL,
				database.FIELD_GAMBAR };
		int[] tujuan = new int[] { R.id.nama2, R.id.gambar2 };
		mCursorAdapter = new SimpleCursorAdapter(this,
				R.layout.item_wisata_budaya, cursor, awal, tujuan);
